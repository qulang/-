...
u8 mf_read(u16 len)
{
	...
	for(i=0;i<len/512;i++)
	{
		res=f_read(file,&buffers[512*i],512,&br);
		if(res)
		{
			xianshi("Read Error",0);
			break;
		}else
		{
			s+=br;
	...
	if(len%512)
	{
		res=f_read(file,&buffers[512*i],len%512,&br);
		if(res)	//读数据出错了
		{
			xianshi("Read Error",0);   
		}else
...
{			    
	...
	xianshi("Begin Write file...",0);
	xianshi("Write data len:%d",len);	 
	res=f_write(file,dat,len,&bw);
	if(res)
	{
		xianshi("Write Error:",res);   
	}else xianshi("Writed data len",0);
	xianshi("wancheng",0);
	return res;
}
